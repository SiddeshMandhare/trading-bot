...
class Tradehull:    
	...
	# ==================== SUPER ORDER FUNCTIONS ====================

	def place_super_order(self, tradingsymbol: str, exchange: str, transaction_type: str, 
						quantity: int, order_type: str, trade_type: str, 
						price: float = 0, target_price: float = 0, 
						stop_loss_price: float = 0, trailing_jump: float = 0) -> str:
		"""
		Place a Super Order (Entry + Target + Stoploss together).
		
		According to Dhan API: POST https://api.dhan.co/v2/super/orders
		
		Returns superOrderId
		"""
		# Validate: For MARKET orders, price MUST be 0
		if order_type.upper() == 'MARKET' and price != 0:
			print(f"⚠️ Warning: MARKET order with price={price}. Setting price=0")
			price = 0  # Force to 0
    
		try:
			# Input validation
			if target_price <= 0:
				raise Exception("target_price is required for Super Orders")
			if stop_loss_price <= 0:
				raise Exception("stop_loss_price is required for Super Orders")
			...
			print(f"   Trailing Jump: {trailing_jump}")
			
			# Get security ID from instrument file
			security_id = self._get_security_id(tradingsymbol, exchange)
			if not security_id:
				print(f"❌ Could not find security ID for {tradingsymbol}")
				return None
			print(f"🔍 Found security ID for {tradingsymbol}: {security_id}")
			
			# Map parameters to Dhan API values
			exchange_map = {
				"NSE": "NSE_EQ",
				"NFO": "NSE_FNO", 
				"BSE": "BSE_EQ",
			...
			# Prepare the payload as per Dhan API
			payload = {
				# "dhanClientId": self.ClientCode,
				# "transactionType": transaction_type_map.get(transaction_type,transaction_type ),
				# "exchangeSegment": exchange_map.get(exchange, exchange),
				# "productType": product_type_map.get(trade_type, trade_type),
				# "orderType": order_type_map.get(order_type, order_type),
				# "securityId": str(security_id),
				# "quantity": str(int(quantity)),  # Convert to string as per API
				# "price": str(float(price)),  # Convert to string
				# "triggerPrice": "0",  # For SL orders
				# "disclosedQuantity": "0",
				# "validity": "DAY",
				# "afterMarketOrder": "false",
				# "amoTime": "OPEN",
				# "boProfitValue": str(target_price) if target_price else "",
            	# "boStopLossValue": str(stop_loss_price) if stop_loss_price else "",
				# "drvExpiryDate": "",
				# "drvOptionType": "",
				# "drvStrikePrice": "",
				# "stoplossPrice": str(stop_loss_price) if stop_loss_price else "",
            	# "targetPrice": str(target_price) if target_price else ""
				 "dhanClientId": self.dhan_client_id,
				 "correlationId": f"SO_{int(time.time())}",  # Optional but helpful
            "transactionType": transaction_type_map.get(transaction_type,transaction_type ),
            "exchangeSegment": exchange_map.get(exchange, exchange),
            "productType": product_type_map.get(trade_type, "INTRADAY"),
            ...
            "price": float(price),  # Float, not string
            "triggerPrice": "0",
            "disclosedQuantity": "0",
            "validity": "DAY",
            "afterMarketOrder": "false",
            "amoTime": "OPEN",
            # Bracket order fields - try different combinations
            #"boProfitValue": str(target_price) if target_price > 0 else "",
            #"boStopLossValue": str(stop_loss_price) if stop_loss_price > 0 else "",
            # Alternative field names
            "drvExpiryDate": "",
            "drvOptionType": "",
            "drvStrikePrice": "",
            # Try additional fields that might trigger bracket order
            "stoplossPrice": float(stop_loss_price),  # REQUIRED
            "targetPrice": float(target_price),  # REQUIRED
			"trailingJump": float(trailing_jump) if trailing_jump > 0 else 0.0  # REQUIRED
			}
			
			# # Add super order specific fields (these are optional)
			# if target_price > 0:
			# 	payload["targetPrice"] = str(float(target_price))
			
			# if stop_loss_price > 0:
			# 	payload["stoplossPrice"] = str(float(stop_loss_price))
			
			# if trailing_jump > 0:
			# 	payload["trailingJump"] = str(float(trailing_jump))

			# TRAILING STOP LOGIC: NO target_price, only stop_loss and trailing_jump
			if trailing_jump > 0:
				# For trailing stop, we need stop_loss_price and trailing_jump
				if stop_loss_price > 0:
					payload["stoplossPrice"] = str(float(stop_loss_price))
					payload["trailingJump"] = str(float(trailing_jump))
					# DO NOT include targetPrice for trailing stops
					print("🎯 Setting up TRAILING STOP order")
				else:
					raise Exception("For trailing stop, stop_loss_price is required")
			else:
				# For fixed stop loss (no trailing)
				if stop_loss_price > 0:
					payload["stoplossPrice"] = str(float(stop_loss_price))
				
				# Only include targetPrice if no trailing
				if target_price > 0:
					payload["targetPrice"] = str(float(target_price))
			# Debug: Print payload (without sensitive data)
			debug_payload = payload.copy()
			debug_payload.pop('dhanClientId', None)
			...
			if response.status_code == 200:
				if response_data.get("status") == "success" or "orderId" in response_data:
					# Dhan API returns orderId not superOrderId
					super_order_id = response_data.get("orderId")
					if not super_order_id:
						# Try data.orderId if response.orderId doesn't exist
						super_order_id = response_data.get("data", {}).get("orderId")
						#Super_order_status = response_data.get('orderStatus', 'TRANSIT')
					
					if super_order_id:
						super_order_status = response_data.get('orderStatus', 'TRANSIT')
						self.logger.info(f"Super order placed successfully. OrderId: {super_order_id} and SuperorderStatus:{super_order_status}")
						return str(super_order_id)
					else:
						raise Exception("No order ID found in response")
				else:
					error_msg = response_data.get("message", "Unknown error")
					error_details = response_data.get("data", {})
					raise Exception(f"Super order placement failed: {error_msg}. Details: {error_details}")

		except Exception as e:
			print(f"❌ Error in place_super_order: {e}")
			self.logger.exception(f"Error in place_super_order: {e}")
	...
	# 		# IMPORTANT: For bracket orders, product type should be "BO"
	# 		product_type_map = {
	# 			"MIS": "BO",  # Changed from "INTRADAY" to "BO" for bracket orders
	# 			"CNC": "DELIVERY",
	# 			"NRML": "MARGIN",
	# 			"CO": "CO",
	# 			"BO": "BO"
	...
	# 		# IMPORTANT: Prepare the payload for BRACKET ORDER (not just super order)
	# 		payload = {
	# 			"dhanClientId": self.dhan_client_id,
	# 			"transactionType": transaction_type_map.get(transaction_type, transaction_type),
	# 			"exchangeSegment": exchange_map.get(exchange, exchange),
	# 			"productType": product_type_map.get(trade_type, "BO"),  # Default to BO for bracket
	# 			"orderType": order_type_map.get(order_type, order_type),
	...
	# 			# BRACKET ORDER FIELDS (must use these exact field names)
	# 			"boOrder": "true",  # CRITICAL: This tells Dhan it's a bracket order
	# 			"boProfitValue": str(target_price) if target_price > 0 else "",
	# 			"boStopLossValue": str(stop_loss_price) if stop_loss_price > 0 else "",
				
	# 			# OPTIONAL: Also send these for compatibility
	# 			"drvExpiryDate": "",
	# 			"drvOptionType": "",
	# 			"drvStrikePrice": "",
				
	# 			# Keep these for backward compatibility (some brokers need them)
	# 			"stoplossPrice": str(stop_loss_price) if stop_loss_price > 0 else "",
	# 			"targetPrice": str(target_price) if target_price > 0 else ""
	# 		}
			
	# 		# For SELL (SHORT) positions, ensure SL is above entry
	# 		if transaction_type == "SELL" and stop_loss_price > 0:
	# 			if stop_loss_price <= float(price):
	# 				print(f"⚠️ For SELL order, SL ({stop_loss_price}) should be above entry ({price})")
			
	# 		print(f"🔍 SUPER ORDER PAYLOAD: {json.dumps(payload, indent=2)}")
			
	# 		# Make API call to BRACKET ORDER endpoint
	# 		headers = {
	# 			'access-token': self.token_id,
	# 			'Content-Type': 'application/json',
	# 			'client-id': self.ClientCode
	...
	# 		# IMPORTANT: Try both endpoints
	# 		endpoints_to_try = [
	# 			"https://api.dhan.co/v2/super/orders",  # Super order endpoint
	# 			"https://api.dhan.co/v2/orders"         # Regular order endpoint
	# 		]
	...
	# 		if response.status_code == 200:
	# 			if response_data.get("status") == "success" or "orderId" in response_data:
	# 				super_order_id = response_data.get("orderId") or response_data.get("data", {}).get("orderId")
					
	# 				if super_order_id:
	# 					print(f"✅ BRACKET ORDER placed successfully. OrderId: {super_order_id}")
	# 					print(f"ℹ️ Note: SL and Target orders may appear as separate child orders")
	# 					return str(super_order_id)
	# 				else:
	# 					raise Exception("No order ID found in response")
	# 			else:
	...
	def modify_super_order(self, order_id: str, order_type: str = None, quantity: int = None,
						price: float = None, target_price: float = None,
						stop_loss_price: float = None, trailing_jump: float = None,
						leg_name: str = None) -> str:
		"""
		Modify a super order or specific leg.
		
		According to Dhan API: PUT https://api.dhan.co/v2/super/orders/{superOrderId}
		"""
		try:
			if leg_name:
				leg_name = leg_name.upper()
			
			# Map leg names to Dhan API values
			leg_map = {
				"ENTRY_LEG": "ENTRY_LEG",
			...
			if order_type:
				...
				payload["orderType"] = order_type_map.get(order_type.upper(), "LIMIT")
			...
			if leg_name:
				payload["legName"] = leg_map.get(leg_name, "ENTRY_LEG")
			...
			if response.status_code == 200 and response_data.get("status") == "success":
				modified_id = response_data["data"].get("orderId", order_id)
				self.logger.info(f"Super order modified successfully. OrderId: {modified_id}")
				return str(modified_id)
			else:
				error_msg = response_data.get("message", "Unknown error")
				raise Exception(f"Super order modification failed: {error_msg}")
				
		except Exception as e:
			print(f"Error in modify_super_order: {e}")
			self.logger.exception(f"Error in modify_super_order: {e}")
			return None
	
	def cancel_super_order(self, order_id: str, leg_name: str = None) -> str:
		"""
		Cancel a super order or specific leg.
		
		According to Dhan API: DELETE https://api.dhan.co/v2/super/orders/{superOrderId}
		"""
		try:
			if leg_name:
				leg_name = leg_name.upper()
			
			# Prepare URL with optional leg parameter
			url = f"https://api.dhan.co/v2/super/orders/{order_id}"
			if leg_name:
				url += f"?legName={leg_name}"
			...
			if response.status_code == 200 and response_data.get("status") == "success":
				cancel_status = response_data["data"].get("status", "CANCELLED")
				self.logger.info(f"Super order cancelled successfully. Status: {cancel_status}")
				return str(cancel_status)
			else:
				error_msg = response_data.get("message", "Unknown error")
				raise Exception(f"Super order cancellation failed: {error_msg}")
				
		except Exception as e:
			print(f"Error in cancel_super_order: {e}")
			self.logger.exception(f"Error in cancel_super_order: {e}")
			return None
	
	def get_super_orders(self, order_id: str = None):
		"""
		Get super orders - all or specific.
		
		According to Dhan API:
		- All orders: GET https://api.dhan.co/v2/super/orders
		- Specific order: GET https://api.dhan.co/v2/super/orders/{superOrderId}
		"""
		try:
			# Prepare URL
			if order_id:
			...
			if response.status_code == 200 and response_data.get("status") == "success":
				...
				error_msg = response_data.get("message", "Unknown error")
				raise Exception(f"Failed to fetch super orders: {error_msg}")
				
		except Exception as e:
			print(f"Error in get_super_orders: {e}")
			self.logger.exception(f"Error in get_super_orders: {e}")
			return []

	def _get_security_id(self, tradingsymbol: str, exchange: str) -> str:
		"""Helper method to get security ID from instrument file."""
		try:
			exchange_map = {
				"NSE": "NSE",
				"NFO": "NSE", 
			...
			if security_check.empty:
				...
				if security_check.empty:
					raise ValueError(f"Symbol '{tradingsymbol}' not found in instrument file")
				else:
					print(f"⚠️ Found {tradingsymbol} but not on exchange {exchange}. Using first match.")
			...
			print(f"❌ Error in _get_security_id: {e}")
			raise